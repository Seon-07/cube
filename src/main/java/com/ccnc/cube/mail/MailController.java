...
//====================================================================================	       보낸 메일함(완)  
	...
	public String sendMailList(Model model, HttpSession session, @RequestParam(defaultValue = "1") int page,
		...
		// List<SendMail> SendMailList = mailService.sentMailBoxList(usersMail,
		// MailIsDelete.존재, MailImportant.기본);
		Page<SendMail> SendMailPage = mailService.sentMailBoxPage(usersMail, MailIsDelete.존재, MailImportant.기본,
				pageable);
...
//====================================================================================	       받은 메일함(완)
	...
	public String receiveMailList(Model model, HttpSession session, @RequestParam(defaultValue = "1") int page,
			@RequestParam(defaultValue = "10") int size) {
		...
		Page<ReceiveMail> filteredPage = new PageImpl<>(filteredMailList.subList(start, end), pageable,
				filteredMailList.size());
		if (page < 1) {
			page = 1;
		}
...
//====================================================================================	       삭제 메일함(완)
	...
	public String sendIsdeleteMailList(Model model, HttpSession session, @PathVariable int type,
		...
		if (type == 1) {
			...
			Page<SendMail> sendMailPage = mailService.sentMailBoxPage(UsersEmail, MailIsDelete.삭제, MailImportant.기본,
					sendPageable);
			...
			Page<ReceiveMail> receiveMailPage = mailService.receivedMailBoxPage(UsersEmail, MailIsDelete.삭제,
					MailImportant.기본, receivePageable);
...
//====================================================================================	     받은  중요 메일함(완)
	...
	public String receiveImportantMailList(Model model, HttpSession session, @PathVariable int type,
			@RequestParam(defaultValue = "1") int page, @RequestParam(defaultValue = "10") int size) {
		...
		if (type == 1) {
			Pageable sendPageable = PageRequest.of(page - 1, size, Sort.by(Sort.Direction.DESC, "sendMailId"));
			Page<SendMail> sendMailPage = mailService.sentMailBoxPage(UsersEmail, MailIsDelete.존재, MailImportant.중요,
					sendPageable);
			if (page < 1) {
				page = 1;
			}
			...
			Pageable receivePageable = PageRequest.of(page - 1, size, Sort.by(Sort.Direction.DESC, "receiveMailId"));
			Page<ReceiveMail> receiveMailPage = mailService.receivedMailBoxPage(UsersEmail, MailIsDelete.존재,
					MailImportant.중요, receivePageable);
			if (page < 1) {
				page = 1;
			}
...
//====================================================================================	       휴지통에서 보낸 메일 완전 삭제 기능
	...
	public String sendMailIsDelete(@PathVariable Integer sendMailId) {
		...
		return "redirect:/isdeletemail_list/1";
	}
//====================================================================================	       휴지통에서 받은 메일 완전 삭제 기능
	...
	public String receiveMailIsDelete(@PathVariable Integer receiveMailId) {
		...
		return "redirect:/isdeletemail_list/2";
	}
...
//====================================================================================	       메일 검색(완)
	...
	public String searchSendMail(@PathVariable String mailType, @PathVariable String searchType,
			@PathVariable String searchInput, Model model, HttpSession session,
			@RequestParam(defaultValue = "1") int page, @RequestParam(defaultValue = "10") int size) {
		Users users = (Users) session.getAttribute("login_user");
...
//====================================================================================	       자동완성 기능
	/*
	 * @Controller public class AutoComController {
	 * 
	 * @Autowired AutoComService service;
	 * 
	 * @GetMapping("/ajax/autocomplete.do") public @ResponseBody Map<String, Object>
	 * autocomplete(@RequestParam("keyword") String keyword) throws Exception {
	 * 
	 * List<Map<String, Object>> resultList = service.autocomplete(keyword);
	 * Map<String, Object> resultMap = new HashMap<>(); resultMap.put("resultList",
	 * resultList);
	 * 
	 * return resultMap; } }
	 */
